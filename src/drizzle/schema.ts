...
export const ProductTable = pgTable(
  ...
  (table) => [index("products_clerk_user_id_index").on(table.clerkUserId)]
);

export const productRelations = relations(ProductTable, ({ one, many }) => ({
  customizations: one(ProductCustomizationTable),
...
export const ProductCustomizationTable = pgTable("product_customizations", {
  ...
  locationMessage: text("location_message").notNull().default(`
        Hey! It looks like you are from <b>{country}</b>. We support Parity Purchasing Power, so if you need it, use code <b>"{coupon}"</b> to get a <b>{discount}%</b>off.`),
...
export const UserSubscriptionTable = pgTable(
  ...
  (table) => [
    index("user_subscriptions_clerk_user_id_index").on(table.clerkUserId),
    index("user_subscriptions_stripe_customer_id_index").on(
